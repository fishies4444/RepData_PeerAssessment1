...
1. First I will load the ggplot2 library.  I will unzip the data file, then read it. 
```{r}
library(ggplot2)
unzip("activity.zip")
...
```
The mean number of steps per day is `r mean1`. The median number of steps per day is `r median1`. 
...
The 5-minute interval with the average maximum number of steps is `r stepsmax`. 
...
```
The mean number of steps per day is `r mean2`. The median number of steps per day is `r median2`. The mean stayed the same while the median went up one. 
